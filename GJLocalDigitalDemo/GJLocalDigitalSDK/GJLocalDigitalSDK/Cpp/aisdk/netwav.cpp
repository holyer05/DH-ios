...
int KWav::initbuf(int pcmsample) {
    ...
    m_wavsample = pcmsample + MFCC_OFFSET+ MFCC_OFFSET_R;
    ...
    m_bnfsize = floor(static_cast<float>(m_melsize - WENET_WINDOW_SIZE) / WENET_STRIDE) + 1;
    ...
    //m_bnfmat->zeros();
    m_bnfblock = std::min<int>(m_duration * MFCC_FPS, m_bnfsize);
    LOGD("==seca %d secb %d\n", m_seca, m_secb);
...
/*
该构造函数的主要功能是根据给定的音频文件初始化 KWav 对象。
它负责打开音频文件、读取文件头信息、计算音频文件的持续时间、初始化缓冲区、读取音频数据并将其转换为浮点数格式。
最后，它确保所有数据已经准备好，并清理临时使用的资源。整个过程为后续的音频处理和特征提取做好了准备。
*/
KWav::KWav(const char* filename, MBnfCache* bnfcache,float duration) {
    ...
    // ======== dump wav ========
/*    ps = (short*)pcmbuf->data();
    std::ofstream csv_file("/data/data/ai.guiji.duix.test/wav.csv");
    for (size_t i = 0; i < sample; i++) {
        csv_file << ps[i] << std::endl;
         }*/
     // ============================
...
void KWav::get_wenet_buffers(JMat** feat_mat, int* n_feat, MBnfCache** bnf_cache) {
    *feat_mat = m_melmat;
    *n_feat = m_melsize;
    *bnf_cache = m_bnfcache;
}

int KWav::calcbuf(int calcinx, float** ppwav, float** ppmfcc, int* pmel) {
