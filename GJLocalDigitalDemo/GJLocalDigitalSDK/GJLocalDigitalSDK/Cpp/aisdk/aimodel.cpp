...
Ort::Env env(ORT_LOGGING_LEVEL_VERBOSE, "onnx_log");
...
int OnnxModel::doRunModel(void** arrin,void** arrout,void* stream,AiCfg* pcfg){
    ...
    try {
        this->session.Run(Ort::RunOptions{nullptr},
            cfg->name_inputs.data(),   // 需确认name_inputs已正确初始化模型输入名称
            inputTensors.data(), incnt,
            cfg->name_outputs.data(),  // 需确认name_outputs已正确初始化模型输出名称
            outputTensors.data(), outcnt
        );
        return 0;
