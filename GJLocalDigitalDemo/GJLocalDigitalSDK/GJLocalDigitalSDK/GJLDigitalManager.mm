...
//初始化多个 dispatch_queue 队列，用于处理并发和串行任务。这些队列主要用于音频播放、图片播放、定时任务等。
- (void)initQueue {
...
-(NSInteger)initBaseModel:(NSString*)basePath digitalModel:(NSString*)digitalPath showView:(UIView*)showView
{
    ...
    //加密路径中的解密文件
    NSString * decry_wenet_onnx_path=[NSString stringWithFormat:@"%@/wenet.onnx",basePath];
    NSString * decry_weight_168u_path=[NSString stringWithFormat:@"%@/weight_168u.bin",basePath];
    
    NSString * decry_paramPath=[NSString stringWithFormat:@"%@/dh_model.param",digitalPath];
    NSString * decry_binPath=[NSString stringWithFormat:@"%@/dh_model.bin",digitalPath];
    NSString * decry_configJson=[NSString stringWithFormat:@"%@/config.json",digitalPath];
    NSString * decry_bboxPath=[NSString stringWithFormat:@"%@/bbox.json",digitalPath];
    ...
    // 加密文件和解密文件的检查与处理
    if (![fileManager fileExistsAtPath:encry_wenet_onnx_path]) {
        if ([fileManager fileExistsAtPath:decry_wenet_onnx_path]) {
            // 解密文件存在，跳过解密并将其移动到目标位置
            NSError *error = nil;
            NSString *targetPath = [NSString stringWithFormat:@"%@/wenet.onnx", decryBasePath];
            if (![fileManager fileExistsAtPath:targetPath]) {
                [fileManager copyItemAtPath:decry_wenet_onnx_path toPath:targetPath error:&error];
                if (error) {
                    NSLog(@"Error copying file: %@", error.localizedDescription);
                } else {
                    NSLog(@"Successfully copied decrypted file to target path.");
                }
            }
        } else {
            // 解密文件不存在，进行解密
    ...
    if (![fileManager fileExistsAtPath:encry_weight_168u_path]) {
        if ([fileManager fileExistsAtPath:decry_weight_168u_path]) {
            // 解密文件存在，跳过解密并将其移动到目标位置
            NSError *error = nil;
            NSString *targetPath = [NSString stringWithFormat:@"%@/weight_168u.bin", decryBasePath];
            if (![fileManager fileExistsAtPath:targetPath]) {
                [fileManager copyItemAtPath:decry_weight_168u_path toPath:targetPath error:&error];
                if (error) {
                    NSLog(@"Error copying file: %@", error.localizedDescription);
                } else {
                    NSLog(@"Successfully copied decrypted file to target path.");
    ...
    if (![fileManager fileExistsAtPath:encry_paramPath]) {
        if ([fileManager fileExistsAtPath:decry_paramPath]) {
            // 解密文件存在，跳过解密并将其移动到目标位置
            NSError *error = nil;
            NSString *targetPath = [NSString stringWithFormat:@"%@/dh_model.param", decryDigitalPath];
            if (![fileManager fileExistsAtPath:targetPath]) {
                [fileManager copyItemAtPath:decry_paramPath toPath:targetPath error:&error];
                if (error) {
                    NSLog(@"Error copying file: %@", error.localizedDescription);
                } else {
                    NSLog(@"Successfully copied decrypted file to target path.");
    ...
    if (![fileManager fileExistsAtPath:encry_binPath]) {
        if ([fileManager fileExistsAtPath:decry_binPath]) {
            // 解密文件存在，跳过解密并将其移动到目标位置
            NSError *error = nil;
            NSString *targetPath = [NSString stringWithFormat:@"%@/dh_model.bin", decryDigitalPath];
            if (![fileManager fileExistsAtPath:targetPath]) {
                [fileManager copyItemAtPath:decry_binPath toPath:targetPath error:&error];
                if (error) {
                    NSLog(@"Error copying file: %@", error.localizedDescription);
                } else {
                    NSLog(@"Successfully copied decrypted file to target path.");
    ...
    if (![fileManager fileExistsAtPath:encry_configJson]) {
        if ([fileManager fileExistsAtPath:decry_configJson]) {
            // 解密文件存在，跳过解密并将其移动到目标位置
            NSError *error = nil;
            NSString *targetPath = [NSString stringWithFormat:@"%@/config.json", decryDigitalPath];
            if (![fileManager fileExistsAtPath:targetPath]) {
                [fileManager copyItemAtPath:decry_configJson toPath:targetPath error:&error];
                if (error) {
                    NSLog(@"Error copying file: %@", error.localizedDescription);
                } else {
                    NSLog(@"Successfully copied decrypted file to target path.");
    ...
    if (![fileManager fileExistsAtPath:encry_bboxPath]) {
        if ([fileManager fileExistsAtPath:decry_bboxPath]) {
            // 解密文件存在，跳过解密并将其移动到目标位置
            NSError *error = nil;
            NSString *targetPath = [NSString stringWithFormat:@"%@/bbox.json", decryDigitalPath];
            if (![fileManager fileExistsAtPath:targetPath]) {
                [fileManager copyItemAtPath:decry_bboxPath toPath:targetPath error:&error];
                if (error) {
                    NSLog(@"Error copying file: %@", error.localizedDescription);
                } else {
                    NSLog(@"Successfully copied decrypted file to target path.");
...
-(void)toStart:(void (^) (BOOL isSuccess, NSString *errorMsg))block
{
    ...
    [self toStopDigitalTime];
    [self playNext];
    self.digitalTimer =[GJLGCDTimer scheduledTimerWithTimeInterval:0.05 repeats:YES queue:strongSelf.digital_timer_queue block:^{
        [strongSelf playNext];
    }];
